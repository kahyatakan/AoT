...
T = TaylorExpansion(
    ...
    order=2                 # 2. dereceye kadar aç
)

# 3. Sembolik sonucu görün
print(T.symbolic)
# Çıktı: x
# (sin(x)'in 0 etrafındaki 2. derece yaklaşımı sadece "x"tir,
#  çünkü x² terimi sin için sıfırdır)
...
### Örnek 5 — Maclaurin serisi (nokta = orijin)

Açılım noktasını vermezseniz, otomatik olarak orijin (sıfır noktası) kullanılır. Buna **Maclaurin serisi** denir:

```python
T = TaylorExpansion(
    f=sp.cos(x),
    variables=[x]       # point verilmedi → otomatik olarak [0]
)
...
```python
TaylorExpansion(f, variables, point=None, order=2)
```

| Parametre | Tip | Açıklama |
|---|---|---|
| `f` | `sympy.Expr` | Açılmak istenen fonksiyon. Sympy ifadesi olmalı. |
| `variables` | `list[sympy.Symbol]` | Fonksiyonun değişkenleri. 1 ile 3 arasında eleman. |
| `point` | `list` veya `None` | Açılım noktası. Sayısal (ör. `[1, 0]`) veya sembolik (ör. `[a1, a2]`) olabilir. `None` verilirse orijin kullanılır. |
| `order` | `int` | Taylor açılımının derecesi. Şu an yalnızca `2` desteklenir. |
...
|---|---|---|
| `.latex()` | `str` | Taylor ifadesinin LaTeX string'i |
...
### Neden sadece 2. derece?

2. derece (kuadratik) yaklaşım, uygulamada en yaygın kullanılandır — optimizasyon algoritmalarının çoğu (Newton metodu gibi) bu seviyede çalışır. Daha yüksek dereceler gelecek sürümlerde eklenecektir.
